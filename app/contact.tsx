...
function Contact() {
    ...
    return (
        <div id='contact' className="h-screen w-full relative bg-black mt-10">
           <ul className='flex justify-around items-center text-white pt-10'>
            {listItems.map((item)=>(
                <LinkButtonContact href={item.link} name={item.title} key={item.title}/>
            ))}
           </ul>
          <div className="full-name absolute bottom-0 left-1/2 transform -translate-x-1/2 flex justify-between px-10 w-full items-center text-[12vw] font-bold">
