...
export default function LinkButton({ href , name }: { href: string; name: string }) {
  ...
  const textSub = gsap.utils.toArray<HTMLElement>(".sub-text");
    ...
    return(
        <li onClick={()=>{gsap.to(window,{scrollTo: href, duration: 2, ease: "power2.out"})}} className="text-[15px] hover-text cursor-pointer opacity-0 text-white uppercase font-bold font-hand overflow-hidden relative">
